      ...
      <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row">
          <div class="col-sm-12 m-b-xs">
            <?php if($dt['level'] !== 'sales') : ?>
              <button type="button" name="expt" id="expt" class="btn btn-success btn-sm exportData" title="Export To Csv." rel="tooltip"><span class="glyphicon glyphicon-export"></span> Export Data</button>
            <?php endif ?>
          </div>
          <div class="col-lg-12">
            <div class="ibox ">
              ...
              <div class="ibox-content">
                ...
                <div class="table">
                  <table class="display table table-hover table-bordered myFppList" data-filter=#filter>
                    <thead>
                      <tr>
                        <th>#</th>
                        <th>No. FPP</th>
                    ...
                    <tbody>
                      <?php
                        ...
                        $sql = "SELECT * FROM master_fpp ORDER BY fpp_id ASC";
                        $mwk = $db1->prepare($sql);
                        $mwk->execute();
                        $resl = $mwk->get_result();
                        $no   = 1;
                      ...
                      <tr>
                        ...
                        <td>
                          <?php 
                            if ($row['approvl'] == '0') {
                              echo '<button class="btn btn-sm btn-info aprove" id-fpp="'.$row["fpp_id"].'">Aprov</button>';
                            } else {
                              echo '<a href="printData.php?id='.$row["fpp_id"].'" class="btn btn-sm btn-primary" target="_blank">Download</a>';
                            }
                          ?>
                        </td>
                        <td>
                          <button class="btn btn-info btn-xs view_data" type="button" name="view" id-fp="<?php echo $row['fpp_id'];?>" title="View Detail" rel="tooltip"><span class="glyphicon glyphicon-eye-open"></span></button>
                      ...
                      <?php } } else { ?>
                      <tr>
                        <td colspan="6">Tidak ada data euy</td>
                      <?php } ?>
                      </tr>
                    </tbody>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content animated bounceInUp">
      ...
      <div class="modal-body">
        <h5>Filter Export</h5>
        <form method="POST" action="exportfpp.php" id="formExport">
          <div class="row">
            <div class="col-sm-6">
            ...
            <div class="col-sm-12">
              <button id="simpan" type="submit" class="btn btn-success pull-right" ><span class="glyphicon glyphicon-download"></span> Download</button>
            </div>
          </div>
        </form>
      </div>
